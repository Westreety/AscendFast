...
当成平级 kind：
- **静态 KV cache**：decode 每步重分配 KV cache 是常见浪费。启用 `StaticCache`
  （或等价）让 decode 不再重分配；注意 cache 容量要覆盖最大 seq len。
- **权重布局**：权重一次性预转 ND→NZ（fractal），让 matmul 跳过每步转换。
- **dtype / 量化**：扫 `model.parameters()` 把残留 fp32 → fp16/bf16，但**保留
  `inv_freq` 等精度关键 buffer 为 fp32**（RoPE 频率精度丢了会坏掉数值）。要改权重
  （如量化）就写**新文件**，别原地改硬链接来的权重。
